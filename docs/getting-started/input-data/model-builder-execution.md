...
| `solve_network`         | Snakemake rule | Solves the model builder for a specific year. This rule is only triggered after either `add_baseyear` or `add_brownfield`. Optimization settings are defined in `config.yaml` (see [model-builder-configuration](model-builder-configuration.md)). |
...
1. `-j1` means running only 1 job in parallel at a time and `-c4` means allowing each job to use up to 4 CPU threads. See the [Snakemake CLI documentation](https://snakemake.readthedocs.io/en/stable/executing/cli.html){:target="_blank"} to customize cores, threads, and parallel execution.
