...
import { BarChart3, CarFront, CircleDollarSign, Cloud, Factory, Menu, Moon, Play, RefreshCw, Search, Settings2, Sun, Zap } from "lucide-react";
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <div className="main-column">
      <header className="topbar"><button className="menu" onClick={() => setSidebarOpen(!sidebarOpen)} aria-label="Open result filters"><Menu aria-hidden="true" /></button><div className="section-tabs top-section-tabs" role="tablist" aria-label="Output sections">{sections.map((item) => { const SectionIcon = sectionIcons[item.id as keyof typeof sectionIcons] || Zap; return <a className={`section-tab ${item.id === section.id ? "active" : ""}`} href={`?section=${item.id}`} key={item.id} role="tab" aria-selected={item.id === section.id} onClick={(event) => { event.preventDefault(); chooseSection(item.id); }}><SectionIcon aria-hidden="true" /><b>{item.label}</b><small>{item.charts.length}</small></a>; })}</div><div className="top-actions"><button className="button secondary" onClick={() => loadCatalog(true)}><RefreshCw aria-hidden="true" />Refresh data</button><a className="button primary" href="/docs" target="_blank">API</a></div></header>
      <main id="workspace">
        <section className="page-title"><h1>{section.title}</h1></section>
        <section className="analysis"><div className="analysis-head"><div><p className="eyebrow">{section.eyebrow}</p><h2>Output visualisations</h2></div><div className="analysis-tools"><label className="search"><Search aria-hidden="true" /><input value={search} onChange={(event) => setSearch(event.target.value)} type="search" placeholder={`Find a ${section.label.toLowerCase()} visualisation`} /></label></div></div>{error && <div className="notice">{error}</div>}<div className={`chart-grid ${selection.comparison ? "comparison-active" : ""}`}>{charts.map((chart) => <ChartCard key={chart.id} chart={chart} selection={selection} years={sector!.years} onYearChange={(year) => setSelection((current) => ({ ...current, year }))} mappings={catalog.mappings} darkMode={dark} onInspect={(title, rows, sourceCount) => setInspector({ title, rows, sourceCount })} />)}</div>{charts.length === 0 && <div className="no-results">No visualisation matches “{search}”.</div>}</section>
      </main>
