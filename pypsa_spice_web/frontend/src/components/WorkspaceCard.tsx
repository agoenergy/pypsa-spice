    ...
    <article className={styles["project-card"]}>
      <header>
        <div className={styles["project-title"]}>
          ...
          <div>
            ...
            <span>{workspace.dataset}</span>
          </div>
        </div>
        {workspace.countries.length > 0 && (
          <span className={styles["country-count"]}>
      ...
      <div className={styles["project-columns"]}>
        <ScenarioList
          ...
          icon={<FileInput />}
          href={(scenario) =>
            `?view=inputs&dataset=${encodeURIComponent(workspace.dataset)}&project=${encodeURIComponent(workspace.project)}&scenario=${encodeURIComponent(scenario)}`
          }
        />
        <ScenarioList
          variant="results"
          ...
          icon={<BarChart3 />}
          href={(run) =>
            `?section=power&dataset=${encodeURIComponent(workspace.dataset)}&project=${encodeURIComponent(workspace.project)}&run=${encodeURIComponent(run)}`
          }
        />
...
function ScenarioList({
  ...
  return (
    <section className={`${styles["scenario-list"]} ${styles[variant]}`}>
      <ListHeader icon={icon} label={label} count={items.length} />
      {items.length ? (
        <div>
          {items.map((item) => (
            <a href={href(item)} key={item}>
              {item}
...
function ProjectDashboardList({
  ...
  return (
    <section className={`${styles["scenario-list"]} ${styles["dashboards"]}`}>
      ...
      {loading ? (
        ...
        <div>
          {dashboards.map((dashboard) => (
            <button type="button" onClick={() => onOpen(dashboard.id)} key={dashboard.id}>
              {dashboard.title}
              <ArrowRight aria-hidden="true" />
            </button>
          ))}
        ...
        <p>
          {error || "No saved dashboards found"}
