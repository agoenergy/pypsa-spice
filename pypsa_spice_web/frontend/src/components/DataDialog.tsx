...
function YearlyDataDialog({ chart, rows, sourceCount, onClose }: Props) {
  ...
  const categoricalOptions = (column: string) =>
    [...new Set(displayTable.rows.map((row) => String(row[column] ?? "")).filter(Boolean))].sort((left, right) =>
  ...
  const updateNumericFilter = (column: string, index: 0 | 1, value: string) => {
    const tableColumn = table.getColumn(column);
    const range = [
      ...((tableColumn?.getFilterValue() as [number | undefined, number | undefined] | undefined) || [
        undefined,
        undefined,
      ]),
      ...
      <section className="dialog yearly-table-dialog" role="dialog" aria-modal="true" aria-labelledby="dialog-title">
          ...
          <div className="yearly-filter-chips" aria-label="Active filters">
            ...
            {columnFilters.map((filter) => {
              ...
              return (
                ...
                >
                  {displayLabel(filter.id)}: {label} <X aria-hidden="true" />
                </button>
              );
            })}
          </div>
        ...
        <div className="table-wrap yearly-table-wrap">
          ...
          >
                    ...
                    return (
                      ...
                      >
                        <div className="yearly-column-heading">
                          ...
                          >
                            <span>
                              {displayLabel(column.id)}
                              {(displayTable.yearColumns.has(column.id) || column.id === "value") && chart.units ? (
                                <small>{chart.units}</small>
                              ) : null}
                            </span>
                          ...
                          <details className={`yearly-column-filter ${filtered ? "active" : ""}`}>
                            ...
                            <div onClick={(event) => event.stopPropagation()}>
                              <header>
                                <b>{displayLabel(column.id)}</b>
                                {filtered && (
                                  <button type="button" onClick={() => column.setFilterValue(undefined)}>
                                    Clear
                                  </button>
                                )}
                              </header>
                                ...
                                <div className="yearly-number-filter">
                                  <label>
                                    ...
                                    <input
                                      type="number"
                                      value={range[0] ?? ""}
                                      onChange={(event) => updateNumericFilter(column.id, 0, event.target.value)}
                                    />
                                  </label>
                                  <label>
                                    <span>Maximum</span>
                                    <input
                                      type="number"
                                      value={range[1] ?? ""}
                                      onChange={(event) => updateNumericFilter(column.id, 1, event.target.value)}
                                    />
                                  </label>
                                </div>
                              ) : (
                                  ...
                                  {categoricalOptions(column.id).map((value) => (
                                    <label key={value}>
                                      <input
                                        ...
                                        onChange={(event) =>
                                          updateCategoricalFilter(column.id, value, event.target.checked)
                                        }
                                      />
                                      <span>{value}</span>
                                    </label>
            ...
            {chart.summary === "sum" && filteredRows.length > 0 && (
              <tfoot>
                <tr>
                  {visibleColumns.map((column, index) => {
                    ...
                    return (
                      ...
                      >
                        {index === 0 ? (
                          <b>Total</b>
                        ) : summaryColumns.includes(column.id) ? (
                          <b>{formatCell(totals[column.id])}</b>
                        ) : null}
                      </td>
                    );
                  })}
        ...
        <footer className="yearly-table-footer">
          <span>
            ...
            {displayTable.pivoted ? ` from ${rows.length.toLocaleString()} values` : ""} ·{" "}
            {sourceCount.toLocaleString()} source rows
          </span>
          <label>
            Rows per page{" "}
