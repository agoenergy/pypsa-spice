...
export default function DashboardChartCard({
  ...
  const missingScenarios = config.scenarios.filter(
    (name) => !project.scenarios.some((scenario) => scenario.name === name),
  );
  const incompatibleScenarios = config.scenarios.filter(
    (name) => !missingScenarios.includes(name) && !eligibleScenarioNames.has(name),
  );
    ...
    Promise.all(
      ...
      .catch((reason) => {
        if (reason.name !== "AbortError")
          setError(reason instanceof Error ? reason.message : "Could not load this chart.");
      })
      .finally(() => {
  ...
  const displayedRows =
    config.mode === "difference" && series.length === 2
  ...
  const toggleScenario = (scenarioName: string) => {
    ...
    update({
      scenarios: selected
        ? config.scenarios.filter((name) => name !== scenarioName)
      ...
      {editing && (
        <div className={styles["dashboard-card-config"]}>
          ...
          <Field>
            ...
            <select
              value={config.mode}
              onChange={(event) => changeMode(event.target.value as DashboardChartConfig["mode"])}
            >
            ...
            <select
              ...
              onChange={(event) => {
                ...
                const available = project.scenarios
                  .filter((scenario) => scenario.sectors.some((candidate) => candidate.name === sector))
                ...
                update({
                  sector,
                  mode,
                  scenarios: selected.length ? selected.slice(0, 2) : available.slice(0, mode === "difference" ? 2 : 1),
                  year: undefined,
                  startTime: undefined,
                  endTime: undefined,
                });
            ...
            {scenarioOptions.map((scenario) => {
              ...
              return (
                <label key={scenario.name}>
                  ...
                  {scenario.missing && <small>Missing · deselect to replace</small>}
                </label>
              );
            })}
          </fieldset>
          ...
          {chart.hourly && (
            <Field>
              ...
              <select
                value={selectedYear}
                onChange={(event) => update({ year: event.target.value, startTime: "", endTime: "" })}
              >
                {years.map((year) => (
          ...
          {!error && hasRows && config.mode === "scenario" && series.length > 1 && (
            <>
              ...
              <ChartLegend
                ...
                onToggle={(value) => toggleLegend(value, setHiddenLegendValues)}
              />
            </>
          )}
          {!error && hasRows && config.mode === "difference" && series.length === 2 && (
            <div className={chartSurfaceStyles["difference-plot"]}>
              ...
              <Plot
                chart={chart}
                primary={series[0].response}
                comparison={series[1].response}
                primaryName={series[0].scenario}
                comparisonName={series[1].scenario}
                mappings={mappings}
                darkMode={darkMode}
                expanded={expanded}
                difference
    ...
    <Field>
      ...
      >
        {sections.map((section) => (
          <optgroup label={section.label} key={section.id}>
            {section.charts.map((chart) => (
              <option value={`${section.id}\u0000${chart.id}`} key={chart.id}>
                {chart.name}
              </option>
            ))}
          </optgroup>
