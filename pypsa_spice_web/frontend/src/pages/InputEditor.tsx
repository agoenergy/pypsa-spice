...
export default function InputEditor({
  ...
  const defaultTable = catalog.global_tables.find((item) => item.id === "Technologies")?.id || definitions[0]?.id || "";
      ...
      {menuTarget &&
        createPortal(
          <nav className={sidebarStyles["submenu-list"]} aria-label="Input pages">
            <button
              className={`${sidebarStyles["submenu-item"]} ${view === "technology" ? sidebarStyles["active"] : ""}`}
              onClick={() => chooseView("technology")}
              aria-current={view === "technology" ? "page" : undefined}
            >
              <Cpu aria-hidden="true" />
              <b>By technology</b>
            </button>
            <button
              className={`${sidebarStyles["submenu-item"]} ${view === "table" ? sidebarStyles["active"] : ""}`}
