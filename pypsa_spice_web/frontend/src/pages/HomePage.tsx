...
export function workspaceInventory(
  ...
  dashboards: DashboardWorkspaceSource[] = [],
  ...
  const ensureWorkspace = (dataset: string, project: string) => {
    ...
    const created: WorkspaceInventoryItem = {
      key,
      dataset,
      project,
      inputScenarios: [],
  ...
  inputCatalog?.datasets.forEach((dataset) =>
  ...
  return [...workspaces.values()].sort(
    (left, right) => left.project.localeCompare(right.project) || left.dataset.localeCompare(right.dataset),
  );
}
...
export default function HomePage({
  ...
  const inventory = useMemo(
    () => workspaceInventory(catalog, inputCatalog, dashboards ?? []),
    [catalog, inputCatalog, dashboards],
  );
      ...
      <section className={styles["home-guide-panel"]}>
        ...
        <ul className={styles["home-workflow"]}>
          <WorkspaceActionCard
            ...
            description="Create the initial project and input-file structure."
            disabled
          />
          <WorkspaceActionCard
            icon={<FileInput />}
            title="Inputs"
            description="Inspect technologies and edit the selected scenario's CSV inputs."
            onClick={() => onNavigate("inputs")}
          />
          <WorkspaceActionCard
            icon={<GitCompareArrows />}
            title="Scenario differences"
            description="Review configuration and input differences before running the model."
            onClick={() => onNavigate("compare")}
          />
          <WorkspaceActionCard
            icon={<Settings2 />}
            title="Configure & run"
            description="Set scenario parameters, review the model scope, and start Snakemake."
            onClick={() => onNavigate("configure")}
          />
          <WorkspaceActionCard
            icon={<BarChart3 />}
            title="Results"
            description="Inspect Power, Industry, Transport, Emissions, and Costs."
            onClick={() => onNavigate("outputs")}
          />
          <WorkspaceActionCard
            icon={<LayoutDashboard />}
            title="Dashboards"
            description="Open, create, and arrange saved result dashboards."
              ...
              <label className={styles["home-project-picker"]}>
                ...
                >
                  {inventory.map((workspace) => (
                    <option value={workspace.key} key={workspace.key}>
                      {workspace.project} · {workspace.dataset}
                    </option>
                  ))}
                </select>
              </label>
          ...
          {!loading && inventory.length === 0 && (
            <div className={styles["home-empty"]}>
              ...
              <span>
                {inputError ||
                  resultError ||
                  "Add a project under the local data directory, then refresh the workspace."}
              </span>
            </div>
          )}
