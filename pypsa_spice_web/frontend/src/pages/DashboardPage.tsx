...
export default function DashboardPage({ catalog, darkMode, onInspect }: Props) {
  ...
  const workspaceOptions = catalog.datasets.flatMap((dataset) =>
    dataset.projects.map((candidate) => ({
  ...
  const chooseProject = (value: string) => {
    ...
    if (
      (next.dataset === dashboard.dataset && next.project === dashboard.project) ||
      (dashboard.rows.some((row) => row.type === "chart") &&
        !window.confirm("Changing project removes all chart rows from this dashboard. Continue?"))
    )
      return;
    setChartPickerOpen(false);
    updateDashboard({
  ...
  const renderChartRow = (row: DashboardChartRow, rowIndex: number, resultProject: Project) => {
    const chart =
      catalog.sections
        ...
        ?.charts.find((candidate) => candidate.id === row.chart.chartId) || null;
    return (
      <DashboardChartCard
        config={row.chart}
        chart={chart}
    ...
    <>
              ...
              <select value={dashboard.id} onChange={(event) => void chooseDashboard(event.target.value)}>
                {summaries.map((summary) => (
                  <option value={summary.id} key={summary.id}>
                    {summary.title} · {summary.chartCount} charts
                  </option>
                ))}
              </select>
            </Field>
      ...
      <section className="dashboard-heading">
        ...
        <label>
          ...
          <textarea
            ...
            onChange={(event) => updateDashboard({ description: event.target.value })}
          />
        </label>
      </section>
      ...
      {!project && (
        <div className="dashboard-project-missing">
          ...
          <span>
            The saved project “{dashboard.project}” is not present in the current Results catalog. Select another
            project above to rebuild this dashboard.
          </span>
        </div>
      )}
            ...
            <section className="dashboard-builder-row dashboard-heading-row" key={row.id}>
              <label className="dashboard-heading-row-field">
                ...
                <textarea
                  ...
                  onFocus={() => {
                    newHeadingId.current = "";
                  }}
                  onBlur={() => {
                    if (!row.title.trim()) updateRow(index, { ...row, title: "Untitled heading" });
                  }}
              ...
              <RowActions
                index={index}
                rowCount={dashboard.rows.length}
                label={`${row.title || "Untitled"} heading row`}
...
function AddChartDialog({
  ...
  const eligibleScenarios = project.scenarios.filter((scenario) =>
    scenario.sectors.some((candidate) => candidate.name === sector),
  );
          ...
          <div className="dashboard-dialog-grid">
            ...
            <Field>
              ...
              <select value={mode} onChange={(event) => changeMode(event.target.value as DashboardChartConfig["mode"])}>
                ...
                <option value="difference" disabled={eligibleScenarios.length < 2}>
                  Difference
                </option>
              </select>
            </Field>
              ...
              return (
                <label key={scenario.name}>
                  <input
                    type="checkbox"
                    checked={checked}
                    disabled={!checked && scenarios.length >= 2}
                    onChange={() => toggleScenario(scenario.name)}
                  />
                  <span>{scenario.name}</span>
                  {checked && mode === "difference" && (
                    <small>{scenarios.indexOf(scenario.name) === 0 ? "Reference" : "Comparison"}</small>
                  )}
                </label>
              );
            })}
...
function ImportDashboardDialog({
  ...
  const missingScenarios = [
    ...new Set(charts.flatMap((row) => row.chart.scenarios).filter((scenario) => !availableScenarios.has(scenario))),
  ];
  const missingCharts = charts.filter(
    (row) =>
      !catalog.sections
        .find((section) => section.id === row.chart.sectionId)
        ...
        <div className="dashboard-import-summary">
          <dl>
            <div>
              <dt>Project</dt>
              <dd>
                {dashboard.project} · {dashboard.dataset}
              </dd>
            </div>
            <div>
              <dt>Charts</dt>
            ...
            <div>
              <dt>Scenarios</dt>
              <dd>{new Set(charts.flatMap((row) => row.chart.scenarios)).size}</dd>
            </div>
          </dl>
          {!project && (
...
function scenarioYearsForPicker(project: Project, scenarios: string[], sector: string): string[] {
  ...
  project.scenarios
    ...
    .forEach((scenario) =>
      scenario.sectors.find((candidate) => candidate.name === sector)?.years.forEach((year) => years.add(year)),
    );
