  ...
  return (
    <PageHeader title={definition.label} className="selection-title">
      <dl>
        <div>
          <dt>Scope</dt>
          <dd>{definition.scope === "global" ? "Global input" : "Scenario input"}</dd>
        </div>
        <div>
          <dt>Sector</dt>
          <dd>{definition.sector || "All sectors"}</dd>
        </div>
      </dl>
    </PageHeader>
  );
...
export default function TableEditor({
  ...
  const load = async (signal?: AbortSignal) => {
    ...
    setSuccess("");
    try {
      const data = await getInputTable(
        selection,
        definition,
        {
          technology,
  ...
  useEffect(() => {
    setPage(0);
  }, [query, filter, country, technology?.id]);
  useEffect(() => {
    const warn = (event: BeforeUnloadEvent) => {
      if (changes.size) event.preventDefault();
    };
  ...
  useEffect(() => {
    if (filter !== "ALL" && !filterOptions.includes(filter)) setFilter("ALL");
                  ...
                  <tr>
                    {table.columns.map((column) => (
                      <th key={column.name}>
                        ...
                        {column.editable && <small>Editable</small>}
                      </th>
                    ))}
                  </tr>
                </thead>
