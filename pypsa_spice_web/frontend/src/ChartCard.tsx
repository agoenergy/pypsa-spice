...
export default function ChartCard({ chart, selection, mappings, onInspect }: Props) {
  ...
  }, [selection.dataset, selection.project, selection.scenario, selection.sector, selection.year]);
  useEffect(() => { setShowDifference(false); }, [selection.comparison]);
  ...
  }, [chart, selection, country, filterValue, startTime, endTime]);
  ...
  return <article className={`chart-card ${expanded ? "expanded" : ""} ${comparing ? "comparing" : ""} ${showDifference ? "showing-difference" : ""}`}>
    ...
    {chart.hourly && availableStart !== null && availableEnd !== null && selectedStart !== null && selectedEnd !== null && <div className="time-range-controls" aria-label={`Time range for ${chart.name}`}>
      <div className="time-range-head"><span>Time range</span>{hasTimeRange && <button type="button" onClick={() => { setStartTime(""); setEndTime(""); }}>Reset</button>}</div>
      <div className="time-range-values" aria-live="polite"><output>{readableTimestamp(selectedStart)}</output><span>to</span><output>{readableTimestamp(selectedEnd)}</output></div>
    ...
    <div className={`chart-body ${!loading && primary && primary.rows.length > 0 ? "with-plot" : ""} ${comparing && !showDifference ? "comparison-layout" : ""}`}>
      {loading && <div className="state"><span className="spinner" />Reading result table…</div>}
      ...
      {!loading && !error && primary && primary.rows.length > 0 && !comparing && <Plot chart={chart} primary={primary} comparison={null} primaryName={selection.scenario} comparisonName="" mappings={mappings} expanded={expanded} />}
      {!loading && !error && primary && primary.rows.length > 0 && comparing && !showDifference && <>
        <div className="scenario-plot"><div className="scenario-label"><small>Primary scenario</small><b>{selection.scenario}</b></div><Plot chart={chart} primary={primary} comparison={null} primaryName={selection.scenario} comparisonName="" mappings={mappings} expanded={expanded} /></div>
        <div className="scenario-plot"><div className="scenario-label"><small>Comparison scenario</small><b>{selection.comparison}</b></div><Plot chart={chart} primary={comparison!} comparison={null} primaryName={selection.comparison} comparisonName="" mappings={mappings} expanded={expanded} /></div>
      </>}
