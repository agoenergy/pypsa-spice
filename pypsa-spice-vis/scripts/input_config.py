...
def handle_base_settings(config, file_path):
    ...
    with st.container(border=True):
        ...
        try:
            years_list = sorted([int(year.strip()) for year in years_text.split("\n") if year.strip()])
            valid_years = [year for year in years_list if 2019 <= year <= 2050]
            if len(valid_years) != len(years_list):
                st.warning("Some years were removed as they were outside the valid range (2019-2050)")
...
def display_co2_limits(config, years):
    ...
    co2_df = pd.DataFrame({
        'Year': years,
        'CO2 Limit': [config["co2_management"]["co2_limit"].get(str(year), 0.0) for year in years]
    })
    edited_co2_df = st.data_editor(
        co2_df,
        column_config={
            "Year": st.column_config.NumberColumn("Year", min_value=2019, max_value=2050, step=1),
            "CO2 Limit": st.column_config.NumberColumn("CO2 Limit (Mt)", min_value=0.0, format="%.1f")
        },
        hide_index=True,
    )
...
def display_emission_costs(config, years):
    ...
    emission_costs_df = pd.DataFrame({
        'Year': years,
        'Emission Cost': [config["co2_management"]["emission_cost"].get(str(year), 0.0) for year in years]
    })
    edited_emission_costs_df = st.data_editor(
        emission_costs_df,
        column_config={
            "Year": st.column_config.NumberColumn("Year", min_value=2019, max_value=2050, step=1),
            "Emission Cost": st.column_config.NumberColumn("Emission Cost (€/tCO2)", min_value=0.0, format="%.1f")
        },
        hide_index=True,
    )
    return {str(row['Year']): row['Emission Cost'] for _, row in edited_emission_costs_df.iterrows()}
...
def handle_co2_management(config, file_path):
    ...
    with st.container(border=True):
        ...
        countries = [key for key in management_config.keys() 
        ...
        selected_country = st.selectbox(
            "Select Country/Region for CO2 management:",
            countries,
            index=0
        )
        ...
        if "value" in country_config:
            ...
            edited_df = st.data_editor(
                ...
                column_config={
                    "Year": st.column_config.NumberColumn("Year", min_value=2019, max_value=2050, step=1),
                    "Value": st.column_config.NumberColumn(f"Value ({unit})", min_value=0.0, format="%.1f")
                },
                hide_index=True,
            ...
            updated_values = {str(row['Year']): row['Value'] for _, row in edited_df.iterrows()}
...
def handle_custom_constraints(config, file_path):
    ...
    with st.container(border=True):
        ...
        # Get list of countries from co2_management (excluding comments)
        countries = [key for key in custom_constraint.keys() 
        ...
        # Select country to edit
        selected_country = st.selectbox(
            "Select Country/Region for custom constraints:",
            countries,
            index=0
        )
        ...
        if country_constraints.get("re_gen_constraint", False):
            ...
            re_gen_df = pd.DataFrame({
                'Year': list(map(int, re_gen_constraint["res_generation_share"].keys())),
                'Fraction': list(re_gen_constraint["res_generation_share"].values())
            })
            edited_re_gen_df = st.data_editor(
                re_gen_df,
                column_config={
                    "Year": st.column_config.NumberColumn("Year", min_value=2020, max_value=2050, step=5),
                    "Fraction": st.column_config.NumberColumn("Fraction", min_value=0.0, max_value=1.0, format="%.2f")
                },
                num_rows="dynamic",
                hide_index=True
            )
...
def config_editor():    
    ...
    file_path = os.path.join(st.session_state.input_data_folder_path,
