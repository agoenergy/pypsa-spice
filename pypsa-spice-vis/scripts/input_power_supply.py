...
########################## Render the UI part ##########################################
    ...
    tech_mapping = dict(zip(
        dfs["tech_df"]["technology"], 
        dfs["tech_df"]["technology_nomenclature"]
    ))
    ...
    with col21:
        ...
        if not selected_type_full:
            st.warning("At least one technology type must be selected. Resetting to default.")
